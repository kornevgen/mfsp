...
void insert_edge_to_2graphs(Graph *graph1, Graph *graph2, int pos, int f, int t)
{
    ...
    //@ assert !graph2->edges[pos].existent; // now proved!
    //@ assert edges_valid(graph2, graph2->ecnt); // now proved!
    //@ assert \forall integer k; graph2->ecnt <= k < graph2->esize ==> !graph2->edges[k].existent; // now proved!
