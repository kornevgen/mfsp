...
/*@
    ...
    assigns graph->edges[pos]; // additional assigns
    // no allocates
    ensures valid(graph);
...
*/    /*@ assert graph->vertices[f2].existent; // assigns graph->edges[..]; is automatically computed for insert_edge! graph->vertices[..] is not changed.
*/    /*@ assert !graph->edges[pos2].existent; // not proved: "pos2" is located at the region of graph->edges
*/    insert_edge(graph, pos2, f2, t2);
}

/*@
    requires \valid(graph1);
    requires valid(graph1);
    requires 0 <= pos < graph1->ecnt;
    requires !graph1->edges[pos].existent;
    requires is_vertex(graph1, f);
    requires is_vertex(graph1, t);
    requires graph1->vertices[f].existent;
    requires graph1->vertices[t].existent;

    requires \valid(graph2);
    requires valid(graph2);
    requires 0 <= pos < graph2->ecnt;
    requires !graph2->edges[pos].existent;
    requires is_vertex(graph2, f);
    requires is_vertex(graph2, t);
    requires graph2->vertices[f].existent;
    requires graph2->vertices[t].existent;

    requires graph1->edges != graph2->edges; // pointers are compared => these are from the same region
    requires \base_addr(graph1->edges) != \base_addr(graph2->edges); // but from the different blocks*/
// */
void insert_edge_to_2graphs(Graph *graph1, Graph *graph2, int pos, int f, int t)
{
    insert_edge(graph1, pos, f, t);
    /*@ assert \valid(graph2); // allocates \nothing; is automatically computed for insert_edge!
*/    /*@ assert graph2->vertices[f].existent; // assigns graph2->edges[..]; is automatically computed for insert_edge! graph2->vertices[..] is not changed.
*/    /*@ assert !graph2->edges[pos].existent; // not proved: "pos2" is located at the region of graph2->edges
*/    /*@ assert edges_valid(graph2, graph2->ecnt); // not proved: region of graph2->edges was changed.
*/    /*@ assert \forall integer k; graph2->ecnt <= k < graph2->esize ==> !graph2->edges[k].existent; // not proved: region of graph2->edges was changed.
*/    insert_edge(graph2, pos, f, t);
}
