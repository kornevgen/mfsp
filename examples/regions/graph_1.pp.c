...
// why graphs[..].edges are located in separate regions?
...
/*@
    ...
    requires \forall integer k; 0 <= k < n ==>
        ...
        (\forall integer k2; 0 <= k2 < n ==> \base_addr(graphs[k]->vertices) != \base_addr(graphs[k2]->vertices)) &&
        (\forall integer k2; 0 <= k2 < n ==> \base_addr(graphs[k]->edges) != \base_addr(graphs[k2]->edges));
    ensures \forall integer k; 0 <= k < n ==>
        graphs[k]->edges[pos].existent &&
        graphs[k]->edges[pos].from == f &&
        graphs[k]->edges[pos].to == t;*/
// */
void insert_edge_to_graphs(Graph **graphs, int n, int pos, int f, int t)
{
    /*@
        loop invariant \forall integer k; 0 <= k < n ==> \forall integer k2; 0 <= k2 < n ==> \base_addr(graphs[k]->vertices) != \base_addr(graphs[k2]->vertices);
        loop invariant \forall integer k; 0 <= k < n ==> \forall integer k2; 0 <= k2 < n ==> \base_addr(graphs[k]->edges) != \base_addr(graphs[k2]->edges);
    ...
    for (int i = 0; i < n; ++i) {
        ...
        /*@ assert n >= 2 && i == 0 ==> \valid(graphs[1]); // 
*/        /*@ assert n >= 2 && i == 0 ==> graphs[1]->vertices[f].existent; // 
*/        /*@ assert n >= 2 && i == 0 ==> !graphs[1]->edges[pos].existent; // 
*/    }
}
...
/*@
    ...
    requires graph->vertices[f].existent;
    requires graph->vertices[t].existent;
    assigns graph->edges[pos]; // assigns was added
    // no allocates
    ensures valid(graph);
    ensures graph->edges[pos].existent;
    ensures graph->edges[pos].from == f;
    ensures graph->edges[pos].to == t;*/
// */
void insert_edge_fixed(Graph *graph, int pos, int f, int t)
{
    graph->edges[pos].existent = 1;
    graph->edges[pos].from = f;
    graph->edges[pos].to = t;
...
/*@
    ...
    ensures graph->edges[pos2].to == t2;*/
// */
void insert_double_edge_fixed(Graph *graph, int pos, int pos2, int f, int t, int f2, int t2)
{
    insert_edge_fixed(graph, pos, f, t);
    /*@ assert \valid(graph); // allocates \nothing; is automatically computed for insert_edge!
*/    /*@ assert graph->vertices[f2].existent; // assigns graph->edges[..]; is automatically computed for insert_edge! graph->vertices[..] is not changed.
*/    /*@ assert !graph->edges[pos2].existent; // now proved: "pos2" is located at the region of graph->edges but it is "not assigns" due to "assigns" clause
*/    insert_edge_fixed(graph, pos2, f2, t2);
}
