...
void qsort(int *a, int l, int u)
{
    if (u <= l) {
        ...
        // partitions the range into two subranges: [l, m - 1] and [m + 1, u]
        int m = partition(a, l, u);
        // assert minimum(a, m, u, a[m]);
        /*@ ghost Lb1:*/

        // sorts the subrange [l, m - 1], not touches the subrange [m + 1, u]
        ...
        // sorts the subrange [m + 1, u], not touches the subrange [l, m - 1]
